...
| Slash commands | Not available | `/help`, `/model`, `/skills`, `/context`, `/clear`, `/quit` |
| One-shot prompts | Not available | `console --prompt "..."` |
| Authoring skills | Not available | Files on disk, or ask the CLI to create one and save it to your organization |
...
The CLI is where [skills](/agents/skills) are authored. It loads both skills and agents from the
filesystem:
...
The loop is quick: write the file, restart, then give the agent a task that should trigger it — the agent
activates a skill itself when its description matches.

Files on disk are loaded only by your CLI sessions. To make a skill available to cloud runs — workflow
steps, and agents that chat dispatches to run against a repository — ask the CLI, with an Amplify API key
configured, to create it and save it to your organization; the built-in `skill-creator` skill handles the
flow, bundling any script you point it at. See [organization skills](/agents/skills#organization-skills).
...
- **No organization skills.** The CLI can list the skills saved to your organization with `list_skills`,
  but it doesn't load or activate them — they run in cloud runs. The CLI activates only the skill files on
  disk.
