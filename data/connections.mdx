...
| **Agent tools** | Letting agents act in Slack, Jira, Confluence, Linear, Notion, Asana, and Google Docs |

Source control is what makes Console able to *act* on your code. Vendor connectors are what let it reason
about findings your existing stack has already produced. Agent tools are what let it put results where your
team works. This page covers the first two; agent tools have [their own section](/agent-tools/overview).
...
<Note>
  ...
  [agent tool](/agent-tools/atlassian), not a vendor connector. Connect Atlassian from the **Agent Tools**
  section of the dialog for that.
</Note>

## Agent tools

The **Agent Tools** section of the Add Connection dialog connects the apps your team works in, so agents
can post to Slack, file Jira or Linear issues, and read and write Confluence, Notion, and Google Docs. An
organization admin connects each app once, as a bot account you own, and every agent in the organization
gets that app's tools.

See [Agent tools](/agent-tools/overview) for what each app exposes, and
[connecting an app](/agent-tools/connecting) for the flow.
...
| Have agents post results to Slack, or file them in Jira or Linear | The matching [agent tool](/agent-tools/overview) |
