...
An agent decides what to do next based on what it just learned. It reads a file, notices a suspicious call,
traces the caller, runs a command to check a hypothesis, and either confirms or discards it. That loop —
reason, act, observe, reason again — suits security work, where the question is usually reachability: can an
attacker actually get there?
