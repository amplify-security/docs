...
<Note>
  ...
  method or body. There is no general-purpose tool for writing to a third-party API. Agents write to other
  systems only through [integration tools](#integrations) for connected apps and through
...
When an organization admin [connects a third-party app](/integrations/connecting), its tools join this list
for every agent in the organization. They're named `<App>_<Action>`, and each app's page lists them in full:

| App | Tools | Examples |
| --- | --- | --- |
| [Slack](/integrations/slack) | 11 | `SlackApi_SendSlackMessage`, `Slack_GetThreadMessages` |
| [Atlassian](/integrations/atlassian) | 36 | `Jira_CreateIssue`, `Jira_SearchIssuesWithoutJql`, `Confluence_CreatePage` |
| [Linear](/integrations/linear) | 20 | `Linear_CreateIssue`, `Linear_TransitionIssueState` |
| [Notion](/integrations/notion) | 10 | `Notion_SearchByTitle`, `Notion_AppendContentToEndOfPage` |
| [Asana](/integrations/asana) | 8 | `Asana_CreateTask`, `AsanaApi_AddTaskComment` |
| [Google Docs](/integrations/google) | 11 | `GoogleDocs_GetDocumentAsDocmd`, `GoogleDocs_CommentOnDocument` |

None of these delete, remove, or archive anything. If an app isn't connected, its tools are absent rather
than failing. See [using integration tools in an agent](/integrations/using-in-agents) for how to grant them and
add a Slack or Jira step to a workflow.
...
| `fan_out_agent` | Dispatches one agent to run against one or more of your projects, each in its own isolated cloud sandbox with a fresh checkout, and returns a summary when every run settles. How a chat reaches other projects' code and [organization skills](/agents/skills#organization-skills). |
...
| Tool | What it does |
| --- | --- |
| `create_skill` | Saves a new [skill](/agents/skills) to your organization from a full `SKILL.md` plus any bundled files, which the CLI reads from your machine. The frontmatter `name` must be free. |
| `list_skills` | Lists the skills this CLI session can activate — labelled `builtin` or `local` — beside the skills saved to your organization. |

Both are **CLI-only** and need an Amplify API key configured. They're how the built-in `skill-creator` skill
does its work; see [organization skills](/agents/skills#organization-skills).
