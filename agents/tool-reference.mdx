...
<Note>
  ...
  systems only through [agent tools](#your-teams-apps) for connected apps and through
...
When an organization admin [connects an app](/agent-tools/connecting), its tools join this list for every
agent in the organization. They're named `<App>_<Action>`, and each app's page lists them in full:

| App | Tools | Examples |
| --- | --- | --- |
| [Slack](/agent-tools/slack) | 11 | `SlackApi_SendSlackMessage`, `Slack_GetThreadMessages` |
| [Atlassian](/agent-tools/atlassian) | 36 | `Jira_CreateIssue`, `Jira_SearchIssuesWithoutJql`, `Confluence_CreatePage` |
| [Linear](/agent-tools/linear) | 20 | `Linear_CreateIssue`, `Linear_TransitionIssueState` |
| [Notion](/agent-tools/notion) | 10 | `Notion_SearchByTitle`, `Notion_AppendContentToEndOfPage` |
| [Asana](/agent-tools/asana) | 8 | `Asana_CreateTask`, `AsanaApi_AddTaskComment` |
| [Google Docs](/agent-tools/google) | 11 | `GoogleDocs_GetDocumentAsDocmd`, `GoogleDocs_CommentOnDocument` |

None of these delete, remove, or archive anything. If an app isn't connected, its tools are absent rather
than failing. See [using agent tools in an agent](/agent-tools/using-in-agents) for how to grant them and
