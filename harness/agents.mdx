...
Everything an agent does in step 2 is bounded by the [tool surface](/harness/tool-reference).
...
Set [`sequential-spawns: true`](/harness/writing-an-agent#sequential-spawns) when steps depend on each other
...
| Source | Description |
| --- | --- |
| **Platform agents** | The library Console ships. See [the agent library](/harness/agent-library). |
...
Prefer a [skill](/harness/skills) instead when you're capturing *how to do one thing well* and an existing
