...
Outputs go to the triggering pull request. To put results in Slack, Jira, Linear, Confluence, Notion, Asana,
or Google Docs, add a final **step** to the chain instead: an agent that consumes the run's findings and
posts them through that app's [integration tools](/integrations/overview). Unlike outputs, such a step runs on
manual runs too.

Two ready-made shapes — one summary message per run, and one issue per finding — are in
[adding a delivery step](/integrations/using-in-agents#adding-a-delivery-step-to-a-workflow).
