...
posts them through that app's [agent tools](/agent-tools/overview). Unlike outputs, such a step runs on
manual runs too.

Two ready-made shapes — one summary message per run, and one issue per finding — are in
[adding a delivery step](/agent-tools/using-in-agents#adding-a-delivery-step-to-a-workflow).
