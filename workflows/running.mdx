...
output already delivered stays delivered. If a [merge gate](/workflows/outputs#gate-merging-on-security-review)
was posted, a cancelled run leaves it blocking
rather than passing.
...
- **A step exceeded its time budget.** Long chains against large repositories can run out of budget.
  ...
  [scanning agents](/agents/library#scanning)) usually resolves it.
